...
| 8 | 3D integration | 🔶 Gate 0 passed 2026-07-16 — the spike's deferred SwapChainPanel/WinUI lifecycle de-risked on the x64 box: composition swapchain bound via `ISwapChainPanelNative`, create-on-show / release-on-hide, `--soak3d` 50 open/close cycles `created=50 released=50 removed=0 errors=0`. Next: productize the renderer (real Grid/ModelColors data, OrbitRig + `tokenbar.orbit.v1`, hover tooltip, 2D/3D toggle) |
| 9 | Polish + parity + vendor re-sync | 🔶 vendor re-sync done early (2026-07-16): crates+vendor at macOS v1.4.0 (`2ed256ee`), cache schema 29, SYNC.md patch table zeroed (rustls + cdylib upstreamed), Interop threads the new hourly/agents `clients` filter · backlog: shell parity for the v1.4.0 client-tabs reorder/hide UI + demo mode (logic layer ready, App wiring pending); optional Windows-only global hotkey to toggle the flyout (no macOS equivalent — needs a key-binding UI) |
