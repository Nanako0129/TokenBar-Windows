...
| Antigravity | Local IDE path passed with `HOME` absent: 8 quota cards, no provider error, and stable card IDs, reset evidence, and `learningDuration` states across repeated polls. The remote OAuth identity path is covered hermetically, not by this live check. |
