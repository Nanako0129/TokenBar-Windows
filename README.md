...
|---|---|
| Claude | Passed with the signed-in Windows profile. |
| Copilot | Passed with the signed-in Windows profile. |
| Antigravity | Passed with `HOME` absent: 8 quota cards, no provider error, and stable card IDs, reset evidence, and `learningDuration` states across repeated polls. |
| Codex | Credential/config discovery and connector code passed, but the installed credential returned `token_invalidated`; this is not a successful live quota fetch. |
